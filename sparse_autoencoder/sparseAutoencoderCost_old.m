...
z_3 = W_b2*a_2;
a_3 = sigmoid(z_3);
size(a_3); %size = 10x5000(10 digits); 64x10000(64 pixels)
size(data); %size = 10000x65

% create y matrice
y_mat = data(:,2:end); % ignore the base
...
tri_2 = zeros(size(W_b2));
tri_1 = zeros(size(W_b1));
...
for t=1:m
    ...
    %1st layer to 2nd(hidden) layer activation
    a_t_1 = data(t,:);  %1x65
    z_t_2 = W_b1*a_t_1';   %25*1 
    a_t_2 = sigmoid(z_t_2); %25*1
    a_t_2 = [1; a_t_2]; %26*1
    
    %2nd layer to 3rd(output) layer activation
    z_t_3 = W_b2*a_t_2; %64*1
    a_t_3 = sigmoid(z_t_3); %64*1
    
    %####Step 2
    this_y = y_mat(t,:)';
    %delta_3 = a_t_3-this_y;
    delta_3 = -(this_y-a_t_3).*sigmoidGradient(z_t_3);  %64*1
    size(delta_3)
    
    %####Step 3
    delta_2 = ((W_b2(:,2:end))'*delta_3 + ...
        sparse_term).*sigmoidGradient(z_t_2); % 25*1;This is for the case when the square error is used in calculation of cost
    %delta_2 = ((W_b2(:,2:end))'*delta_3).*sigmoidGradient(z_t_2); % This
    %is for the cost function when KL is used
    size(delta_2)
    
    %######Step 4
    tri_2 = tri_2 + delta_3*a_t_2'; %64*26
    tri_1 = tri_1 + delta_2*a_t_1;  %25*65
    break;
end

%#####Step5;
% DO not regularized the bias term
Theta1_grad = 1/m*tri_1 + lambda/m*W_b1;
Theta2_grad = 1/m*tri_2 + lambda/m*W_b2;
% no regularization on the bias term
Theta1_grad(:,1) = Theta1_grad(:,1) - lambda/m*W_b1(:,1);
Theta2_grad(:,1) = Theta2_grad(:,1) - lambda/m*W_b2(:,1);
% check size
