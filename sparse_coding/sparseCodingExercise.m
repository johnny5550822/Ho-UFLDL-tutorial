...
%!!!!!!! Non-topographic sprase coding works well, but the topographic coding does not work well.
%  Instructions
%  ------------
% 
...
gamma = 1e-3;   % L2-regularisation parameter (on basis)
...
weightMatrix = randn(visibleSize, numFeatures) * 0.005; %size:64x5
featureMatrix = randn(numFeatures, numPatches) * 0.005; %size:5x6
...
% correct.
groupMatrix = round(rand(100, numFeatures));   %100x5
...
% Initialize matrices
weightMatrix = rand(visibleSize, numFeatures);  %64x121
featureMatrix = rand(numFeatures, batchNumPatches); %121x2000
...
groupMatrix = zeros(numFeatures, donutDim, donutDim);   %121x11x11
...
for iteration = 1:200                      
    ...
    featureMatrix = reshape(featureMatrix, numFeatures, batchNumPatches);   %featureMatrix:121x2000                                      
